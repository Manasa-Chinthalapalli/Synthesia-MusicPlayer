...
const ControlArea = ({playerState, playerControls }) => {
  ...
  //const currentTime = 0;
  //const duration = 180;
  //why we need authentication? for favourite section we need
  ...
  const handleLike = async() => {
    ...
    try {
      ...
      dispatch(updateFavourites(res.data)); //update the redux stored data
    } catch (error) {
    ...
    <div className="control-root">
      ...
      <div className="control-buttons">
...

       {
        isAuthenticated && (
          ...
          {isLiked ? (
            <FaHeart color="#e01010" 
          size={22} />
        ) : (
          <FaRegHeart color="#a855f7" 
          size={22} />
        )}
        </button>
        )}
      ...
      <div className="control-progress-wrapper">
        <input
          ...
          style={{
            background: 
            `linear-gradient(to right,#a855f7 ${duration ? (currentTime/duration) * 100 : 0}%, #333 ${duration ? (currentTime/duration) *100 : 0}%)`
          }}
        />
